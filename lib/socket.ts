...
// Socket.IO disabled — stub API kept so existing imports keep compiling.
// Every accessor reports "no connection" and teardown is a no-op.
export function getSocket(): null {
    return null;
}

export function peekSocket(): null {
    return null;
}
